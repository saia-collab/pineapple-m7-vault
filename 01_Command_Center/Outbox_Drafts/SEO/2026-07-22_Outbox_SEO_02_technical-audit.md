...
| C3 | **Word "free" present in home body copy** | `/` (and possibly other pages — sample crawl only) | grep returned `true` for `\bfree\b` in homepage HTML | Replace every instance with "free roof inspection". Run `brand_firewall.py --check` after. |
...
|---|---|---|---|---|
| I1 | **OG image is a single shared `social.png`** with no per-page override | `/` (and any page without a custom og:image) | `og:image = https://pineappleroofingllc.com/wp-content/uploads/2026/05/social.png` | Add a per-page og:image on at least the city hubs (Frisco, McKinney, Lewisville, Plano). Use the IKO + free roof inspection hero card. |
...
| I5 | **Single H1 is correct, but H2 order is off** | `/` | H2 sequence starts with "License #03-0637" (a credential, not a section), then the actual section "Professional Roofing Services in Dallas, TX Built Around Your Needs" | Reorder: H1 (Frisco TX Roofers…) → H2 (Roofing Services in Frisco) → H2 (Credentials bar: License #03-0637, IKO Certified, 5-Star, Since 2021). |
...
2. **Replace every "free" with free roof inspection** (C3) — single sweep on `/` plus a regex pass on every staged page. Brand-fw check after.
...
6. **Add per-page og:image** on the 8 city pages (I1) — uses the same IKO + free roof inspection hero card.
...
## Handoff
- On-Page Copywriter: home page is the #1 priority. Fix title + H1 + the 2 "free" instances + add IKO + free roof inspection. One pass.
