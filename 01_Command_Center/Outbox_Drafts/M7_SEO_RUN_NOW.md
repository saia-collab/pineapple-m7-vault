...
status: PAUSED
type: seo_run_now
title: M7 — SEO RUN NOW (Agent Kanban / Hermes Goal Mode — striking-distance, migration-aware)
created: 2026-07-21
---
...
## 🧭 THE MIGRATION RULE (read once)
Your rankings live on **pineapplecontractors.com** (old). The new home is **pineappleroofingllc.com**.
So every page the agents write is **built for the NEW domain**, and after you publish you **301 the old URL → new**:
```
old: pineapplecontractors.com/service-areas/<county>/<city>/   →   new: pineappleroofingllc.com/locations/<city>-tx/
```
(That's already in `M7_Redirect_Map.csv`. Publish new → import 301 → keep old domain live.)

## ⚠️ WHY YOUR GOALS FAILED (and the fix)
- **Codex = `spawn codex ENOENT`** → the Codex CLI isn't installed, so any goal routed to Codex dies. **Fix:**
  run these on **Hermes `seo` profile → Kimi K3** (your "coding plan ready" model), NOT the Codex route.
- The failed goal depended on skill files ("Corey copywriting + ai-seo skills + product_marketing.md") that
  the agent couldn't load. **Fix:** the goals below are **self-contained** — every SEO rule is IN the goal,
  so nothing has to be found on disk.

---

## ▶️ RUN FIRST — paste into Agent Kanban "Assemble board" (or Hermes Goal Mode), one at a time
**Target #1 — Allen (already pos 8.8, one push from page 1):**
```
Write a Location Service Page for Pineapple Roofing targeting the keyword "flat roofing allen tx".
...
**Target #3 — Euless (pos 16.8, 85 impressions):** same goal, keyword `"euless roofing"`, URL `/locations/euless-tx/`, file `euless-tx.html`.
**Target #4 — Brand (pos 10 for "pineapple"):**
```
...
> positions climb in OpenSEO. Then repeat with the next 16 striking-distance keywords (export from OpenSEO).

---

## 🤖 HAND THE REST TO CHATGPT / CODEX (once Codex is installed) or Free Claude Code
```
...
> (no API key needed). Or just use ChatGPT web + Free Claude Code — same prompt.

---

## 🎯 RUN ANY PLAYBOOK TASK VIA HERMES GOAL MODE (universal)
...
## ✅ CHECKLIST
- [ ] Run Targets #1-4 on Hermes **seo profile → Kimi K3** (NOT Codex).
- [ ] Brand-check each draft → publish to pineappleroofingllc.com.
- [ ] Import the 301s (`M7_Redirect_Map.csv`) so the old rankings transfer.
