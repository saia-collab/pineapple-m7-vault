...
type: outbox_draft
status: PAUSED
title: 30-Day Marketing Gameplan — Pineapple Contractors (Roofing)
audience: Saia / authorized operator
horizon: 30 days from activation
...
sources_skipped:
  - M7_CONTENT_FACTORY.md — NOT FOUND in workspace; not used as a source.
activation_required: YES — this is a plan; ad spend and live posting stay manual.
---

# 30-DAY MARKETING GAMEPLAN — PINEAPPLE CONTRACTORS (ROOFING)

> **One page. One week at a time. One hero action per day.**
>
> Two things are NOT on this plan because they are reflexes, not tasks:
> 1. **Every lead gets a text or call within 5 minutes.** (Speed-to-lead. Anything older = dead.)
> 2. **Every finished customer gets the same-day review-request text.** (See `2026-06-22_Outbox_ReviewRequest_SameDay_*.md`.)
>
> Those two run every day, automatically, regardless of what week you're in.
...
## WEEK 2 — LOCAL FAN WARM-UP (Days 8–14)
**Focus:** One Meta ad set. The Local Fan avatar (Frisco 35–65 near The Star).
...
## WEEK 3 — AEO CONTENT STARTS (Days 15–21)
...
- **Day 20 — Saturday.** Apply the **1% Kill Rule**: any creative below 1.0% CTR after 48 hours or 1,000 impressions → PAUSE it. Apply the **1.5% Scale Rule**: anything above 1.5% CTR with CPL under $50 → bump budget 15%.
- **Day 21 — Sunday.** Rest. Reflections only.

---

## WEEK 4 — COMPOUND & DECIDE (Days 22–30)
**Focus:** Double down on what worked. Cut what didn't.

- **Day 22 — Monday.** Re-aim your Local Fan budget: shift dollars toward the source with the **lowest cost per signed job** in Attribution (not lowest cost per click).
- **Day 23 — Tuesday.** Repeat the AEO loop: 3 more questions from NotebookLM → 3 more FAQ answers on a **different** service-area page (McKinney or Plano — pick the city with the best week-3 traffic).
- **Day 24 — Wednesday.** GBP post #2 of the week + upload 5 fresh photos from `02_Media_Vault`.
- **Day 25 — Thursday.** Pull a 30-day review report: how many new 5-stars? Reply to anything unanswered.
- **Day 26 — Friday.** Review-request reflex. Update the Attribution sheet — total the 30-day cost per signed job, by source.
- **Day 27 — Saturday.** Decide the next 30 days: which source gets the most budget? Which avatar (Local Fan, Culture Seeker, Founder's Circle) actually booked jobs?
- **Day 28 — Sunday.** Rest.
- **Day 29 — Monday.** Ask me to draft the next 30 days based on what worked. (I'll stage everything PAUSED in `Outbox_Drafts/` — you decide what goes live.)
- **Day 30 — Tuesday.** One-line win: "In 30 days, our cost per signed job dropped from $X to $Y." Put that number on a sticky note where you can see it.

---

## DAILY REFLEXES (run every day, never "forget")

1. **Every lead → reply in 5 minutes.** Older than that = dead lead.
2. **Every finished job → same-day review-request text.** Use the PAUSED templates.

If only the reflexes happen on a given day, the day still counted.

---

## WHAT I WILL DO WHEN YOU ASK

- Draft ad copy, FAQ answers, GBP posts, and review templates — all PAUSED in `Outbox_Drafts/`.
- Run `brand_firewall.py` against anything headed for disk.
- Score leads and flag the ≥80 hot ones for dispatch to Saia.
- Pull weekly numbers from your Attribution sheet on request.

## WHAT I WILL NEVER DO WITHOUT YOUR EXPLICIT "GO"

- Publish live to Meta or Google.
- Move money, flip a campaign, or change a budget.
- Send a text or email to a real customer.
- Edit any file in `01_Command_Center/MASTER_PLAYBOOK.md`, `GROUNDING.md`, or the other governance files.
