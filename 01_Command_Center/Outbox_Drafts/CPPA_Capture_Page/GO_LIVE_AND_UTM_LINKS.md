...
## STEP 1 — Turn on the lead pipe (7 min)
1. Open (or create) your CRM Google Sheet.
2. **Extensions → Apps Script** → delete the sample code → paste everything from
   `CRM_AppsScript_Code.gs` (in this folder).
3. Set `SHEET_ID` = the long ID from your Sheet URL
...
```
const FORM_ENDPOINT = "PASTE_YOUR_APPS_SCRIPT_OR_FORMSPREE_URL_HERE";   ← your /exec URL
const GA4_ID = "G-XXXXXXX";                                             ← your GA4 ID (optional)
```
...
- Row must show your UTM values. **If nothing lands, do NOT run ads yet.**
...
**exactly which reel produced which booked audit** — and then make more of that one. That's the difference
between "posting content" and running a measurable lead engine.
