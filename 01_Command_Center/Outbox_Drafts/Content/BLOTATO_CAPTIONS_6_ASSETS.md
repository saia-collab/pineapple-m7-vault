---
title: Blotato-Ready Captions — the 6 media assets in 03_Marketing_Reels_Pool
status: PAUSED — Outbox Shield. Saia's GO before posting.
brand_check: CPPA (no "free") · IKO Certified (no GAF) · RCAT #03-0637 · 972-928-0788 · zero green
last_updated: 2026-07-22
---

# 📲 BLOTATO CAPTION PACK — your 6 real assets

**How to use:** open Blotato → upload the file → paste the caption → select platforms → **Schedule** (not publish) → Saia hits GO.
**Every CTA = phone + a LIVE page link.** Phone converts fastest; the link builds SEO.

---

## 1️⃣ `2432 W 6th St, Irving, TX.mp4` — drone / roof deck + underlayment
**Best for:** proof-of-process. This is your "we don't cut corners" asset.
**Platforms:** Instagram Reels · Facebook Reels · TikTok · LinkedIn

> This is the part of your roof you'll never see again. 🏠
>
> Synthetic underlayment, fully sealed, before a single shingle goes on. It's what stands between a North Texas storm and your ceiling — and it's where cheap crews cut corners.
>
> We document every layer, so you know exactly what you paid for.
>
> IKO Certified · RCAT License #03-0637 · family-owned since 2005
> 📞 Book your Complimentary Professional Photo Audit — (972) 928-0788
> 🔗 https://pineappleroofingllc.com/roof-shingle-repair-grand-prairie-tx/
> #DFWRoofing #RoofReplacement #IrvingTX #ThePineappleStandard #RoofingMadeSweeter

---

## 2️⃣ `4605 Birkshire Ln.mp4` — job site
**Best for:** neighborhood proof.
**Platforms:** Instagram · Facebook · GBP

> Another roof done right on Birkshire. 🔨
>
> IKO Certified install, site cleaned daily, every step documented for your file. If your street took the same storm, your roof deserves the same look.
>
> 📞 (972) 928-0788 — Complimentary Professional Photo Audit
> 🔗 https://pineappleroofingllc.com/roofing-storm-restoration-frisco-tx/
> #DFWRoofing #RoofReplacement #IKOCertified #RoofingMadeSweeter
> ⚠️ *Confirm the city before posting and swap the link to the matching city page.*

---

## 3️⃣ `5104 Mohawk Dr.mp4` — job site (already in your 30-day tracker, Day 2)
**Best for:** the 3-step process post.
**Platforms:** Instagram Reels · Facebook Reels · TikTok

> A new roof in 3 steps — and you never climb a ladder. 🪜
>
> 1️⃣ We photograph and document every issue (your CPPA).
> 2️⃣ We write the scope and meet your adjuster on site.
> 3️⃣ IKO Certified crew installs it. Daily cleanup. 50-year product warranty.
>
> You handle nothing. We handle the roof *and* the paperwork.
> 📞 (972) 928-0788 · RCAT #03-0637
> 🔗 https://pineappleroofingllc.com/roofing-storm-restoration-frisco-tx/
> #Roofing #DFW #InsuranceClaim #RoofingMadeSweeter

---

## 4️⃣ `Poly fest 2026.mp4` — GRATITUDE POST (festival we sponsored + hosted)
**Best for:** community goodwill + trust. **This is a thank-you, not an ad — keep the sell soft.**
**Platforms:** Instagram · Facebook · TikTok · GBP

### Main caption (Instagram / Facebook)
> To everyone who came out to the Polynesian Festival in Frisco this May — **thank you.** 🍍
>
> Watching our community fill that space with music, dance, food, and family is something we'll carry all year. Pineapple Roofing was proud to sponsor and host it — but the day belonged to **you**: the performers, the vendors, the volunteers, and every family who showed up.
>
> This is the part people don't see behind a roofing company. Before we're contractors, we're a Polynesian family that lives here, raises our kids here, and shows up here.
>
> **Mālō 'aupito.** Thank you, Frisco. We'll see you at the next one.
>
> *Ko e hala 'o e fononga ko e faka'apa'apa.*
>
> 🍍 Pineapple Roofing · family-owned since 2005 · (972) 928-0788
> #PolynesianFestival #FriscoTX #PolynesianOwned #FamilyOwned #ThePineappleStandard #DFW

### Short version (TikTok / Reels)
> Thank you, Frisco. 🍍
> Our community showed up for the Polynesian Festival this May — and we were honored to sponsor and host it.
> Before we're roofers, we're family. Mālō 'aupito.
> #PolynesianFestival #FriscoTX #PolynesianOwned #FamilyOwned

### LinkedIn version
> This May, Pineapple Roofing sponsored and hosted the Polynesian Festival in Frisco.
>
> We're a family-owned, Polynesian-owned contractor serving North Texas since 2005 — and investing in the community we live in isn't marketing to us, it's the reason the business exists.
>
> Thank you to every performer, vendor, volunteer, and family who made the day what it was. Mālō 'aupito.
>
> Pineapple Roofing · RCAT #03-0637 · IKO Certified · (972) 928-0788

### Google Business Profile version (short)
> Thank you to everyone who joined us at the Polynesian Festival in Frisco this May — proudly sponsored and hosted by Pineapple Roofing. Before we're roofers, we're family. Mālō 'aupito. 🍍 (972) 928-0788

> 💡 **Post note:** no hard CTA on purpose. A thank-you that pivots to a sales pitch reads as insincere — the goodwill *is* the conversion. Pin this one; it's the post people screenshot and share.
> ⚠️ Confirm the festival's official name before posting if it has one (e.g. "___ Polynesian Festival") and tag the performers/vendors you worked with.

---

## 5️⃣ `property managers.mp4` — commercial
**Best for:** your highest-ticket audience.
**Platforms:** LinkedIn · Facebook

> Property managers: one storm, every building at risk. ⛈️🏢
>
> We assess your whole portfolio fast, document each roof for insurance, and execute to one standard across every property. 72-hour storm response for priority partners.
>
> IKO Certified · RCAT License #03-0637 · 20 years in DFW
> 📞 (972) 928-0788 — Complimentary Professional Photo Audit for your portfolio
> 🔗 https://pineappleroofingllc.com/commercial-hail-damage-portfolio-frisco-tx/
> #PropertyManagement #CommercialRoofing #HOA #DFW

---

## 6️⃣ `why pineapple roofing.MP4` — your brother on camera ⭐ BEST ASSET
**Cut this 3 ways** (see Reel_Repurpose_Plan). Lead with the insurance hook:

> Your insurance will pay for a new roof after hail. But only if you prove the damage in time. 💧
>
> Most homeowners miss it, because hail damage is invisible from the ground. We climb up, photograph every hit, and hand you a report your adjuster can't wave off.
>
> The name on our truck is the name on our license. RCAT #03-0637 · IKO Certified.
>
> 📞 (972) 928-0788 — Complimentary Professional Photo Audit
> 🔗 https://pineappleroofingllc.com/hail-damage-roof-repair-frisco-tx/
> #HailDamage #InsuranceClaim #FriscoRoofing #DFW #RoofingMadeSweeter

---

## 🎯 LEAD CAPTURE — where the traffic should land
**Right now the strongest CTA is the phone** — it converts fastest and needs zero setup.
**Second-best:** the matching **live city page** (links above — all verified live).

**Your dedicated capture page** (`Outbox_Drafts/Landing_Pages/cppa-capture-page.html`) is built but **not deployed and its form isn't connected.** To use it as the link:
1. Wire the form to Formspree (free) or your CRM — say the word and I'll do it.
2. Deploy it to `pineappleroofingllc.com/cppa/`.
3. Then swap every 🔗 above to that one URL and track conversions in GA4.

**Until then: phone + city page. Don't send traffic to a dead form.**

---

## 📅 SUGGESTED POSTING ORDER (2 weeks)
| Post | Asset | Day |
|---|---|---|
| 1 | why pineapple roofing (insurance hook) | Tue |
| 2 | Poly fest (culture) | Thu |
| 3 | 2432 W 6th St (underlayment proof) | Sat |
| 4 | 5104 Mohawk (3 steps) | Tue |
| 5 | property managers (commercial) | Thu |
| 6 | 4605 Birkshire (neighborhood) | Sat |

Ko e hala 'o e fononga ko e faka'apa'apa.

<!-- M7-FIREWALL-EXEMPT: content-draft -->
