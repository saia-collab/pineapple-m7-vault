# -*- coding: utf-8 -*-
"""Allen flat-roof video: 7 HyperFrames sub-compositions. Portrait 1080x1920.
Same frame IDs/durations as the hail project so index.html assembles unchanged."""
import os
W, H = 1080, 1920
...
F = []
# 1 — HOOK (4s)  VO: "Water doesn't run off a flat roof. It waits."
F.append(("frame-01-hook", 4, NAVY, f'''
    <div class="frame-01-hook-serif" id="f1-a" style="color:{PAPER};font-size:132px;">WATER DOESN'T<br>RUN OFF.</div>
    <div class="frame-01-hook-serif" id="f1-b" style="color:{GOLD};font-size:150px;font-style:italic;margin-top:34px;">It waits.</div>
...
# 2 — QUIET FAILURE (9s)  VO: "Ponding, seam separation, failed flashing... that's how low-slope roofs fail. Quietly."
F.append(("frame-02-bruise", 9, NAVY, f'''
    <div class="frame-02-bruise-eyebrow" id="f2-eb" style="color:{CYAN};margin-bottom:40px;">Low-Slope Failure</div>
    <div class="frame-02-bruise-serif" id="f2-h1" style="color:{PAPER};font-size:126px;">A flat roof<br>doesn't fail loud.</div>
    <div class="frame-02-bruise-serif" id="f2-h2" style="color:{GOLD};font-size:132px;font-style:italic;margin-top:28px;">It fails quiet.</div>
    <div id="f2-bar" style="width:560px;height:80px;background:rgba(0,191,255,.12);border-radius:8px;margin-top:64px;position:relative;overflow:hidden;">
      <div id="f2-crack" style="position:absolute;top:0;left:0;height:100%;width:100%;background:rgba(0,191,255,.22);transform-origin:left center;"></div>
    </div>
    <div id="f2-sub" style="color:rgba(247,245,239,.7);font-size:30px;margin-top:34px;max-width:660px;">Ponding, seam separation, flashing at every HVAC curb.</div>
''', '''  tl.fromTo("#f2-eb",{opacity:0,y:24},{opacity:1,y:0,duration:.5},0.2);
  ...
  tl.fromTo("#f2-crack",{scaleX:0},{scaleX:1,duration:1.6,ease:"power1.inOut"},4.9);
  tl.fromTo("#f2-sub",{opacity:0,y:24},{opacity:1,y:0,duration:.6},6.3);''', ""))

# 3 — HAIL ON MEMBRANE (12s)  VO: "When hail hits a membrane, it doesn't crack. It bruises."
...
F.append(("frame-03-goup", 12, NAVY, f'''
    <div class="frame-03-goup-serif" id="f3-h" style="color:{PAPER};font-size:120px;">Hail doesn't<br>crack a membrane.</div>
    <div class="frame-03-goup-serif" id="f3-h2" style="color:{GOLD};font-size:132px;font-style:italic;margin-top:24px;">It bruises it.</div>
    <div class="frame-03-goup-eyebrow" id="f3-sub" style="color:{CYAN};margin-top:44px;">A pitched-roof crew walks right past it</div>
    <div id="f3-grid" style="display:grid;grid-template-columns:1fr 1fr;gap:26px;margin-top:64px;width:560px;">{photos}</div>
''', '''  tl.fromTo("#f3-h",{opacity:0,y:60},{opacity:1,y:0,duration:.7,ease:"power4.out"},0.2);
  tl.fromTo("#f3-h2",{opacity:0,y:34},{opacity:1,y:0,duration:.6,ease:"power3.out"},1.9);
  tl.fromTo("#f3-sub",{opacity:0,y:24},{opacity:1,y:0,duration:.5},3.4);
  tl.fromTo(".f3-photo",{opacity:0,scale:.88},{opacity:1,scale:1,duration:.5,stagger:.35,ease:"power2.out"},4.6);''',
    ".f3-photo{aspect-ratio:16/10;border-radius:6px;}"))

# 4 — THE FILE (12s)  VO: "We document the whole assembly — field, seams, drains, curbs..."
rows = [("Field","membrane & ponding"),("Seams","lifted laps & welds"),("Drains","penetration flashing"),("HVAC curbs","storm-date proof")]
rows_html = "".join(f'''<div id="f4-r{i}" class="f4-row" style="border-bottom:1px solid rgba(247,245,239,.18);">
  ...
  <span class="frame-04-file-serif" style="color:{PAPER};font-size:52px;">{a}</span>
...
F.append(("frame-04-file", 12, NAVY, f'''
    ...
    <div class="frame-04-file-serif" id="f4-h" style="color:{PAPER};font-size:104px;">We document<br>the whole roof.</div>
    <div id="f4-list" style="margin-top:56px;width:680px;display:flex;flex-direction:column;">{rows_html}</div>
...
# 5 — ZERO DOWNTIME (10s)  VO: "Then we phase the work so your tenants never stop operating."
F.append(("frame-05-keep", 10, NAVY, f'''
    <div id="f5-seal" style="width:200px;height:200px;border:2px solid {GOLD};border-radius:50%;display:flex;align-items:center;justify-content:center;color:{GOLD};font-family:'DM Sans';font-weight:700;font-size:26px;letter-spacing:.14em;text-transform:uppercase;line-height:1.3;margin-bottom:56px;">Phased<br>Work</div>
    <div class="frame-05-keep-serif" id="f5-h" style="color:{PAPER};font-size:118px;">Tenants never<br>stop working.</div>
    <div id="f5-sub" style="color:rgba(247,245,239,.72);font-size:34px;margin-top:40px;">We phase the job around your operation.</div>
    <div class="frame-05-keep-serif" id="f5-q" style="color:{GOLD};font-size:72px;font-style:italic;margin-top:56px;">Downtime is the real cost.</div>
...
# 6 — CREDENTIALS (8s)  VO: "TPO. PVC. Modified bitumen. IKO Certified. RCAT 03-0637."
creds = [("TPO &middot; PVC &middot; Mod-Bit","Commercial low-slope systems"),("IKO Certified","Documented workmanship"),("RCAT #03-0637","Texas licensed &middot; since 2005")]
creds_html = "".join(f'''<div id="f6-c{i}" class="f6-cred">
  <div class="frame-06-cred-serif" style="color:{PAPER};font-size:70px;">{a}</div>
...
# 7 — CTA (5s)  VO: "Book your Complimentary Professional Photo Audit. (972) 928-0788."
