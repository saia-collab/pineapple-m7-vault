...
# Resolve the newest installed pack instead of pinning a dated folder. This was
# hardcoded to agent-os-pack-2026-06-29 while the launchers run 2026-07-30, so an
# update rewrote a folder nothing executes and still reported success.
$TPL = "$ROOT\03_Knowledge_Mat\00_Atlas\templates"
$APP = (Get-ChildItem "$TPL\agent-os-pack-*" -Directory -ErrorAction SilentlyContinue |
        Sort-Object Name -Descending |
        ForEach-Object { Join-Path $_.FullName "source" } |
        Where-Object { Test-Path (Join-Path $_ "package.json") } |
        Select-Object -First 1)
if (-not $APP) {
  Write-Host "No installed agent-os-pack-*\source found under $TPL." -ForegroundColor Red
  Read-Host "Enter to exit"; exit 1
}
Write-Host "Target install: $APP"

# The studio owns 3737. Port 3000 belongs to SEO Office -- 01_Command_Center\PORT_MAP.md.
$PORT = 3737
...
# 6. Start + verify
Start-Process -NoNewWindow powershell -ArgumentList "-Command","`$env:PORT=$PORT; npm start"
Start-Sleep 8
$code = 0
try { $code = (Invoke-WebRequest "http://127.0.0.1:$PORT/" -UseBasicParsing -TimeoutSec 5).StatusCode } catch { $code = 0 }
if ($code -eq 200) { Write-Host "Dashboard UP on http://localhost:$PORT (updated + M7-branded)." -ForegroundColor Cyan }
...
# Ports per 01_Command_Center\PORT_MAP.md -- 3000 is SEO Office, not the studio.
$svc = [ordered]@{ "3737"="Agentic OS"; "3939"="CommandCtr"; "3100"="Paperclip"; "8082"="FCC"; "9119"="Hermes"; "11434"="Ollama" }
