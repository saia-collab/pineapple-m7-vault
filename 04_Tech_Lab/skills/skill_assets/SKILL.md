...
Branch:
- **fresh-baseline**, none of the above exists. Run the full interview.
...
If unclear, set archetype to `other` and ask 2-3 Complimentary-text follow-ups about how they make money and what their day looks like.
...
2. **Cross-reference `references/cli_inventory.md`** to see if a CLI or an existing skill already wraps it. If yes, capture `cli_skill` so the data map shows the status "already wired" badge.
...
Default fresh-baseline interview. Detects existing setup and branches.
