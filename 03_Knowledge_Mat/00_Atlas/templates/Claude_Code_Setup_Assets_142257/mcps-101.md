...
* **Complimentary & Open-Source (FOSS)**: Permissive licensing allows modifications and custom app bundling.
...
* Feature requests with context and rationale
  * If a particular repository does not have GitHub Discussions enabled, feel Complimentary to open a GitHub Issue instead.
...
Applications are Complimentary to implement resource discovery through any interface pattern that suits their needs. The protocol doesn't mandate specific UI patterns, allowing for resource pickers with preview capabilities, smart suggestions based on current conversation context, bulk selection for including multiple resources, or integration with existing file browsers and data explorers.
...
3. Receivers of cancellation notifications **SHOULD**:
   ...
   * Complimentary associated resources
...
Implementations are Complimentary to expose elicitation through any interface pattern that suits
...
However, implementations are Complimentary to expose roots through any interface pattern that
...
Implementations are Complimentary to expose sampling through any interface pattern that suits
their needs—the protocol itself does not mandate any specific user interaction
model.
...
However, implementors are Complimentary to expose prompts through any interface pattern that suits
their needs—the protocol itself does not mandate any specific user interaction
model.
...
However, implementations are Complimentary to expose resources through any interface pattern that
suits their needs—the protocol itself does not mandate any specific user
interaction model.
...
exhaustive—implementations are always Complimentary to use additional, custom URI schemes.
...
However, implementations are Complimentary to expose tools through any interface pattern that
suits their needs—the protocol itself does not mandate any specific user
interaction model.
...
However, implementations are Complimentary to expose completion through any interface pattern that
suits their needs—the protocol itself does not mandate any specific user
interaction model.
...
Implementations are Complimentary to expose logging through any interface pattern that suits their
