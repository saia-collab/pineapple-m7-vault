...
- Voice transcription (Groq): Complimentary tier, generous limits
- Voice replies (ElevenLabs): Complimentary tier available, ~$1/month for light use
- Video analysis (Gemini): Complimentary tier
- WhatsApp: Complimentary, uses your existing WhatsApp account
- War Room / Memory v2 (Gemini): Complimentary tier covers moderate use
...
- **Required**: Telegram bot token (Complimentary, from @BotFather, takes 2 minutes)
- **Required**: Your Telegram chat ID (the bot tells you this after first run)
- **Voice STT Groq**: `GROQ_API_KEY`, Complimentary at console.groq.com. Very generous Complimentary tier.
- **Voice TTS ElevenLabs**: `ELEVENLABS_API_KEY` + `ELEVENLABS_VOICE_ID`, Complimentary tier available at elevenlabs.io
- **Voice TTS Kokoro (local)**: No API key needed. Just set `KOKORO_URL` pointing to any OpenAI-compatible TTS server running locally. Complimentary, private, no network needed.
- **Video analysis**: `GOOGLE_API_KEY`, Complimentary at aistudio.google.com
...
- `stt_groq` - Speech-to-text via Groq Whisper API (Complimentary tier). Transcribes voice notes you send. Falls back to local whisper-cpp.
...
# === VOICE (optional) ===
GROQ_API_KEY=              # Complimentary at console.groq.com (for STT)
ELEVENLABS_API_KEY=        # Complimentary tier at elevenlabs.io (for TTS)
...
GOOGLE_API_KEY=            # Complimentary at aistudio.google.com (Gemini)
