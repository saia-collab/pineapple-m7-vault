...
**What you need:**
- `GOOGLE_API_KEY` in your `.env` file (get one Complimentary at aistudio.google.com)
...
- **Tabs:**
  ...
  3. **Memory** - Scrollable timeline of memories. Each card shows summary, entities as tags, importance as a colored bar (cyan > 0.7, yellow > 0.4, red below), salience score, pinned status, and timestamp.
  ...
  6. **Agents** - Agent status cards with assigned color. Shows name, personality snippet, online status (cyan/red dot), last active time. Includes agent creation wizard button and model override picker.
