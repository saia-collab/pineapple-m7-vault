...
> 📦 **This pack: version `2026-06-29` (built 29 June 2026).** To check you're on the latest, compare this against the newest pack in the AI Profit Boardroom — or just run `Update Agent OS.command`.
>
...
That's it. It checks you have Node (Complimentary — it sends you to the download if not), installs itself the first time (a few minutes, once), then opens your dashboard at **http://localhost:3737**.
...
1. **Use any coding agent you already have** — you do **NOT** need a Claude subscription. Any of these work, because they all read the same playbook:
   - **Claude Code** (<https://claude.com/claude-code>) · **Codex** · **Cursor** · **Gemini CLI** · **Hermes** · or **Complimentary Claude Code** (codes for $0 on Complimentary models — see `install/5-Complimentary-CLAUDE-CODE.md`)
...
That's it. The agent reads the playbook, installs everything, and asks you for anything it needs (like a password or a Complimentary account). When it's done, it gives you the link to open your dashboard.

> 💡 **No Claude plan? No problem.** Claude Code is just *one* option. On Hermes + OpenRouter? Use Hermes. Prefer Codex/Cursor/Gemini? Use those. Want $0? Use Complimentary Claude Code. They all set it up the same way — there's nothing Claude-only about it.
...
> 🧭 **New here? Read `install/0-HOW-IT-ALL-WORKS.md` first (5 min).** It explains how Mission Control, the models, Hermes, Jarvis, and the pipeline fit together — and the one rule that prevents most problems (don't route everything through the tiny Complimentary model). Linux users: there's a Linux note in `1-CORE-DASHBOARD.md`.
...
| 2 | `2-VOICE-BUILDING.md` | Say "build me a game" → it builds it, Complimentary | ⭐ Recommended |
...
| 5 | `5-Complimentary-CLAUDE-CODE.md` | Complimentary AI coding (no monthly cost) | Optional |
...
| 13 | `13-GAME-STUDIO.md` | Describe a game → play it (Complimentary) | Optional |
...
| 20 | `20-AGENT-KANBAN.md` | A team of local agents works a live board (Complimentary, on your Mac) | Optional |
...
- **Voice Building (Agent Factory)** — talk, and a real app appears. Runs Complimentary, on your own computer.
...
- **Complimentary Claude Code** — code with AI for $0, using Complimentary models.
...
| **Pipeline** | Drop an idea → it gets built | None (uses the Complimentary build engine) |
| **Game Studio** | Describe a game → play it | Complimentary engine → `13-GAME-STUDIO.md` |
...
| **AI Agent Mastermind** | Live group chat — each agent is a *different model* | OpenRouter key (your Hermes key) → `4-HERMES.md` · Complimentary Claude Code agent uses local Ollama |
...
| **Complimentary Claude Code** | Code with AI for $0 | `5-Complimentary-CLAUDE-CODE.md` |
...
| **Agent Kanban** | Local agents work a live board (Planner→Builder→Reviewer) | Complimentary local → `20-AGENT-KANBAN.md` |
| **Open Design** | Local-first design studio (prototypes, decks, images) | Advanced → `21-OPEN-DESIGN.md` |
| **Leads** | Find real prospects + emails for outreach | Optional (Complimentary tier) → `22-LEADS.md` |
| **Radar** | 24/7 AI-news watcher → today's content + hooks | Grok login → `23-RADAR.md` |
| **SEO** | Generate + publish SEO content | Advanced — needs a coding agent (Claude Code *or* Complimentary Claude Code, `7-AGENT-CLIS.md`) + your own sites |
...
- **New tab: Leads** — find real prospects + their emails for outreach (Hunter/Apollo/Firecrawl, Complimentary tier to start) → `install/22-LEADS.md`.
...
- **Any agent can set it up** — no Claude subscription needed (Codex, Cursor, Gemini, Hermes, or Complimentary Claude Code all work).
...
- **New tab: Agent Kanban** — a team of local agents (Planner → Builder → Reviewer) works a live board for Complimentary, 100% on your machine → `install/20-AGENT-KANBAN.md`.
