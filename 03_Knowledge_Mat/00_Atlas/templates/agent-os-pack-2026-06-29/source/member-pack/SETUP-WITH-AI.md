...
7a. **🛑 NEVER build or recreate a feature that already exists.** Every tab and sub-tab in the dashboard (Hermes Astros, Hermes Oracle, Radar, Leads, the whole SEO section, etc.) is **already fully built** in `source/src` — with its real, polished UI. Your job is to *connect keys and start services*, **never to write a feature from scratch.** If a tab looks missing, empty, or unstyled, the pack is **out of date** — tell the user to download the newest Agent OS pack (the `build <date>` shows under the "Agentic OS" logo, top-left), then re-run. **Do NOT hand-code a replacement** (e.g. a plain "YouTube search" in place of the real Astros starchart) — that produces a broken lookalike and is always the wrong move.
