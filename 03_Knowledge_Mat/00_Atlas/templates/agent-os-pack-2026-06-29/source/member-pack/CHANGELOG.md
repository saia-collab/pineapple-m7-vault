...
## 2026-06-24
- ✨ **New tab: Leads** — find real people to reach out to. Describe your ideal customer (or paste a domain/CSV list) and it returns scored contacts with emails, ready to feed your outreach. Start **Complimentary** (Hunter Complimentary tier + CSV); add Apollo/Firecrawl for more volume. See `install/22-LEADS.md`.
...
- 📘 **Clarified how the AI Agent Mastermind works** — the group-chat tab runs each agent as a *different model via your OpenRouter key* (the one from your Hermes setup), with the Complimentary Claude Code agent running on your local Ollama. The tab list used to say "no setup" — it actually needs your OpenRouter key. *(Thanks Ridz for the question.)*
...
## 2026-06-21
- 🔧 **Gemini tab marked retired** — Google **retired the Gemini CLI on 18 June 2026** (Complimentary/Pro/Ultra) and replaced it with **Antigravity**. The Gemini tab + guide now say so and point you to the **Antigravity** tab (its direct successor). *(The optional Live-Translate Gemini API key is separate and still works.)*
...
## 2026-06-19
- ✨ **New tab: Agent Kanban** — give it a goal and a little team of **local, offline** agents works a live board for you: a **Planner** breaks it into cards, a **Builder** builds each one, a **Reviewer** checks it really landed — and every Done card previews live. Complimentary, 100% on your machine (uses the same local engine as voice building). See `install/20-AGENT-KANBAN.md`.
...
- 📘 **New: "How It All Works" guide** — a 5-minute read that finally explains how Mission Control, the models, Hermes, Jarvis, and the pipeline fit together — and the #1 rule that prevents most problems: **the small Complimentary model (Gemma2) is only for the Complimentary on-device builder — don't route Hermes, video, or coding agents through it.** If your agents felt flaky or your videos came out as text/blobs, this is almost always why. See `install/0-HOW-IT-ALL-WORKS.md`. *(Thanks Michael for the detailed write-up.)*
...
## Earlier
- The foundation: the dashboard, Voice Building (Agent Factory), the Jarvis voice, Hermes, Complimentary Claude Code, Paperclip (your AI company), and plain-English install guides for every part.
