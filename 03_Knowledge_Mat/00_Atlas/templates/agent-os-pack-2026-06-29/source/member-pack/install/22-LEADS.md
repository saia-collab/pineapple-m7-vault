...
You can start **Complimentary** and only add paid sources if you want more volume.
...
- **Domains** — paste a list of company domains; **Hunter** finds the contact emails (Hunter has a Complimentary tier).
...
- **Hunter** (`HUNTER_API_KEY`) — email finder. **Complimentary tier** to start: <https://hunter.io>
- **Apollo** (`APOLLO_API_KEY`) — people/company database (paid): <https://www.apollo.io>
- **Firecrawl** (`FIRECRAWL_API_KEY`) — site scraping for enrichment (Complimentary tier): <https://firecrawl.dev>
...
2. Pick a source (start with **Domains** or **CSV** — both are Complimentary).
...
## Good to know
- **Start Complimentary.** Hunter's Complimentary tier + CSV + Domains get you going with no spend.
