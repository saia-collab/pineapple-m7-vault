...
Before you install anything, here's the mental model. It makes everything else click — and it saves you the #1 mistake people make (routing everything through the tiny Complimentary model).
...
- **Claude tab** → the real Claude (needs a paid Claude login). The strongest brain for coding. No paid plan? Use **Complimentary Claude Code** instead (below).
- **Complimentary Claude Code (FCC)** → "Claude for $0." It quietly routes the Claude tool to a **Complimentary cloud model** (e.g. N2) so you can code without a subscription. This is your everyday coding brain if you're on Complimentary.
...
- **Gemma2 (or qwen-coder)** → a **small model that runs on your own computer** (via Ollama), for **Complimentary, offline building** only. It powers the "say build me a game → it appears" magic without any API key.

## ⚠️ The one rule that prevents 90% of problems

**Gemma2 is ONLY for the Complimentary, on-device builder.**

Do **not** make Gemma2 the default for Hermes, for video, or for your coding agents — and don't let an AI assistant "helpfully" set it as the global default (it sometimes tries). Gemma2 is a small model: great for quick Complimentary builds, but too weak to drive Hermes' tools or author a real video. Point the heavy jobs at a strong model instead:

- **Coding / engineering agents** → Complimentary Claude Code on **N2** (or real Claude).
...
> 💻 **On a modest laptop?** Lean on the **cloud** (N2 + OpenRouter are Complimentary/cheap) for anything heavy. Local models like Gemma2 are slow on older CPUs — keep them for light builds, or route builds through N2 too.
...
- **Complimentary Claude Code** → reads `~/.fcc/.env` for its model. **Hermes** → reads `~/.hermes/profiles/<name>/config.yaml`. These are separate — set each one.
...
2. Pick **one** brain and get it working end-to-end: Complimentary Claude Code on N2 (`5-Complimentary-CLAUDE-CODE.md`) or Hermes with an OpenRouter key (`4-HERMES.md`).
