// Apollo daily/weekly briefing engine.
//
...
// LLM pass (the same MiniMax/OpenRouter path Apollo chat uses) adds the APOLLO
...
import { complete } from "@/lib/hermesApollo";
...
// "the Pineapple Standard" / "AIPB" (a product, not a figure).
...
// Ask the model for the APOLLO-voice synthesis. Returns parsed JSON or null.
async function synthesize(range: BriefingRange, ctx: string): Promise<{
  ...
  const persona =
    "You are APOLLO — my refined British AI butler — delivering my " +
    ...
    "\"the Pineapple Standard\" / \"AIPB\" is fine — it's a product, not a figure.)\n" +
