...
const SHOW_AGENTS = [
  ...
  { n: "Free Claude", c: "#00BFFF" }, { n: "Apollo", c: "#22d3ee" },
];
...
export default function ApolloView() {
  ...
  const [realtime, setRealtime] = useState(false);   // OpenAI Realtime speech-to-speech mode — default OFF (free: Live + Wake word use the browser's built-in SpeechRecognition; spoken replies use the browser's built-in speechSynthesis — no API key, no cost)
  ...
  async function speak(text: string) {
    // FREE voice path: browser built-in speechSynthesis — no API key, no per-character cost.
    // Replaces the old OpenAI TTS fetch (provider:"openai") so spoken replies cost nothing.
    try {
      if (typeof window === "undefined" || typeof window.speechSynthesis === "undefined") {
        setPhase("idle"); resumeAfterReply();
        return;
      }
      const synth = window.speechSynthesis;
      // Cancel anything still mid-utterance so back-to-back replies don't overlap.
      try { synth.cancel(); } catch {}
      const u = new SpeechSynthesisUtterance(text.slice(0, 600));
      // Try to pick a pleasant English voice; fall back to whatever the browser defaults to.
      ...
      u.pitch = 1.0;
      u.volume = 1.0;
      u.onend = () => { setPhase("idle"); resumeAfterReply(); };
      u.onerror = () => { setPhase("idle"); resumeAfterReply(); };
      setPhase("speaking");
      synth.speak(u);
